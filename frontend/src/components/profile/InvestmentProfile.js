  ...
  const fetchCoin = async () => {
    try {
      const response = await fetch(
        "https://api.coinlore.net/api/ticker/?id=32360",
        {
          method: "GET",
          headers: {
      ...
      setCurrency({
        price_usd: investments > 0 ? investments / investmentQuantity : 0,
      });
    }
  };
        ...
        <>
          ...
          <InvestmentChange>
            ...
            <ValueChange percent={difference < 0}>
              ...
              {percentDifference.toFixed(2)}% ({difference.toFixed(2)}$)
            </ValueChange>
          </InvestmentChange>
          <SellButton onClick={onToggleConfirm}>Sell investment</SellButton>
          <Dialog open={openConfirm} onClose={onToggleConfirm}>
            <ConfirmDialog>
              Are you sure you want to sell your investment? You have{" "}
              {difference > 0 ? "made" : "lost"} {difference.toFixed(2)} $
              {percentDifference >= 10
                ? "You have increased your investment with more than 10% so you will also gain 1 i ranking!"
                : ""}
              <ButtonBox>

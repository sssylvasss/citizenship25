...
const authenticateCitizen = async (req, res, next) => {
  try {
    ...
    if (!citizen) {
      console.log('Auth middleware - Citizen not found or token mismatch');
      throw new Error();
    }
...
app.get('/citizens', async (req, res) => {
  ...
  const sortCitizens = (sortParam) => {
    if (sortParam === 'mostBadges') {
      return { badges: -1 };
    } else if (sortParam === 'senior') {
      return { createdAt: 1 };
    } else if (sortParam === 'junior') {
      return { createdAt: -1 };
    } else if (sortParam === 'richest') {
      return { coins: -1 };
    } else if (sortParam === 'highscoreSpaceball') {
      return { highscoreSpaceball: -1 };
    } else if (sortParam === 'highscoreFish') {
      return { highscoreFish: -1 };
    } else if (sortParam === 'highscoreMath') {
...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    // Generate JWT token
    const token = jwt.sign({ userId: newCitizen._id }, jwtSecret, {
      expiresIn: '7d',
    });
...
app.post('/signin', passwordLimiter, async (req, res) => {
  ...
  try {
    ...
    // Generate JWT token
    const token = jwt.sign({ userId: citizen._id }, jwtSecret, {
      expiresIn: '7d',
    });
...
app.post('/citizenmessage/:userid', async (req, res) => {
  ...
  try {
    ...
    const newCitizenMessage = await new CitizenMessage({
      message,
      user,
    }).save();
...
app.patch('/citizen/:id/badges', authenticateCitizen, async (req, res) => {
  const { id } = req.params;
  try {
